...
import Graphics.Collage exposing (collage, outlined, rect, solid, toForm, circle, solid)
...
putInBox e =
  let (sx,sy) = sizeOf e
  in layers [e, collage sx sy [outlined (solid black) (rect (toFloat sx+6) (toFloat sy+6))]]
...
main =
  ...
  in Signal.map (\dict -> flow down [ button
                                    , collage 200 200 (List.map
                                      (\(p,b) -> Graphics.Collage.move p (toForm b)) (Dict.values dict))
                                    ])
                (foldp update Dict.empty (merge (Signal.map Add (foldp (\_ t -> t + 1) 0 add.signal))
